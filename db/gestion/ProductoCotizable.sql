...
BEGIN
    UPDATE PRODUCTO SET PRO_ESTADO = 1 WHERE ID_PRO = c_id_coti;
    DELETE FROM PRODUCTO_COTIZADO WHERE ID_COTI = c_id_coti;
END;
...
CALL EliminarProductoCotizado(1);

DELIMITER //

CREATE PROCEDURE EditarProductoCotizado(
    IN c_id_coti INT,
    IN nuevo_precio_unitario FLOAT(2),
    IN nuevo_material VARCHAR(70),
    IN nuevas_medidas VARCHAR(70),
    IN nueva_caducidad DATE,
    IN nuevo_aprobado BIT
)
BEGIN
    UPDATE PRODUCTO_COTIZADO
    SET 
        PRO_PRECIOUNITARIO = nuevo_precio_unitario,
        PRO_MATERIAL = nuevo_material,
        PRO_MEDIDAS = nuevas_medidas,
        PRO_CADUCIDAD = nueva_caducidad,
        PRO_APROBADO = nuevo_aprobado
    WHERE ID_COTI = c_id_coti;
END;

//
DELIMITER ;
CALL EditarProductoCotizado(1, 15.5, 'Nuevo Material', 'Nuevas Medidas', '2023-12-31', 1);
